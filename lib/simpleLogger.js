...
function instantiate(clazz, name, prefix) {
	var logger = Object.create(clazz.prototype);
	clazz.call(logger, name, prefix);
	//logger.init();
	return logger;
}
...
function extend(fn, logger, proto) {
	_.each(proto, function(f, fName){
		if(_.isFunction(logger[fName])) {
			fn[fName] = function() {
				var args = Array.prototype.slice.call(arguments);
				return logger[fName].apply(fn, args);
			}
		} else {
			fn[fName] = logger[fName];
		}
	});
}

module.exports.getLogger = function(name, prefix, clazz, extendPrototypes) {
	clazz = clazz || SimpleLogger;
	extendPrototypes = extendPrototypes || [];

	extendPrototypes.push(SimpleLogger.prototype);

	var logger = instantiate(clazz, name, prefix);

	var ret = function(message) {
		var args = Array.prototype.slice.call(arguments);
		args.push(4);
		ret.log.apply(ret, args);
		//ret.log(message, 4);
	};
	...
	//console.log("ret.level", ret.level, logger.level, logger.loggerName);
