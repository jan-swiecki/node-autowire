...
CodeMutator.prototype.reset = function() {
}
...
CodeMutator.prototype.mutateRequire = function(path, mutate) {
  const regex = /\.[a-zA-Z0-9\-]{1,4}$/;
  if(!path.match(regex)) {
    path += '.js';
  }
  
  const code = mutate(fs.readFileSync(path, 'utf8'));
  const tmp_filename = `${PATH.basename(path).replace(regex, '')}_${Math.random().toString(36).substring(2)}.js`;
  let tmp_path;
  try {
    tmp_path = PATH.join(PATH.dirname(path), tmp_filename);
    fs.writeFileSync(tmp_path, code, 'utf8');
    return require(tmp_path);
  } finally {
    if(tmp_path) {
...
CodeMutator.prototype.eval = function(callbackString) {
  if(this.scope) {
    ...
    var callback = this.injector.applyMap(tmpCallback, undefined, this.scope);
    return callback;
  } else {
