...
ModuleFinder.prototype.discoverModuleByName = function(paramName) {
  ...
  if(paramName.indexOf('_') !== -1) {
    var ps = paramName.split('_');
    var p = ps.shift();
    var subPath = ps.join('/');
    absPath = self.nameCache[p];
    if(absPath) {
      log.trace("[%s] Trying sub module \"%s\"", self.parentModuleName, paramName);
  ...
  function _return(ret) {
    if(! _.isUndefined(ret)) {
      ...
      log.warn("[%s] Returning undefined, module \"%s\" not found", self.parentModuleName);
    }
    return ret;
  }
...
ModuleFinder.prototype.generateNameCache = function() {
  ...
  var rootPath = findProjectRoot(self.currentPath);
  if(! rootPath) {
        ...
        if(fs.lstatSync(dirAbsPath).isDirectory()) {
          ...
          if(fs.existsSync(packageJsonPath)) {
            ...
            if(moduleName && moduleMain) {
              ...
              store(nameMapper(moduleName), absPath);
            }
