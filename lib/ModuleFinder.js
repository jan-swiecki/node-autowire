...
ModuleFinder.prototype.setContextPath = function(contextPath) {
    var self = this;
    log.trace("setContextPath", contextPath);
    self.contextPath = contextPath;
	self._contextName = PATH.parse(self.contextPath).base;
	return self;
};
	...
	var importPaths = _(extendedImportPaths)
		.map(function(p) {
			if(PATH.isAbsolute(p)) {
				...
				return PATH.join(self.contextPath, p);
			}
		})
