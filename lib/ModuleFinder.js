...
/**
 * So it can be replaced with another requirer
 */
ModuleFinder.prototype.require = function(name) {
  return require(name);
}
...
ModuleFinder.prototype.generateNameCache = function() {
  ...
  log("[%s] [generateNameCache] Project root \"%s\"", self.parentModuleName, rootPath);

  if(ModuleFinder.globalNameCache[rootPath]) {
    log("[%s] [generateNameCache] Found cached nameCache for current project root", self.parentModuleName);
    self.nameCache = _.assign(self.nameCache, ModuleFinder.globalNameCache[rootPath]);
    self.moduleCache = _.assign(self.moduleCache, ModuleFinder.globalModuleCache[rootPath]);
    self.alias = _.assign(self.alias, ModuleFinder.globalAliases[rootPath]);
  ...
  function mapAllNames(path) {
    log("[%s] [generateNameCache] Mapping %s", self.parentModuleName, path);
  ...
  function mapNodeModules(rootPath) {
    ...
    if(fs.existsSync(nodeModulesPath)) {
      log("[%s] [generateNameCache] Mapping node_modules at \"%s\"", self.parentModuleName, nodeModulesPath);
