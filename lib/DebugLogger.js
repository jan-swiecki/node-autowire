...
DebugLogger.prototype.withPrefix = function(prefix) {
  this.prefix = prefix;
  this.initIfNeeded();
  return this;
};
...
module.exports.getLogger = function(name, depth, prefix) {
  ...
  // compute prefix
  if(! prefix) {
    let parentParentModule = ModuleHelper.getParentModule(depth + 2);
    prefix = SimpleLogger.resolvePrefix(parentParentModule);
  }

  // compute name
  var parentModule = ModuleHelper.getParentModule(depth + 1);
  if(! parentModule) {
    throw new Error("parentModule is undefined, too big depth: "+depth);
  }
  ...
  var logger = SimpleLogger.getLogger(name, prefix, DebugLogger);
