...
Injector.prototype.addImportPath = function(path) {
  this.moduleFinder.addImportPath(path);
  return this;
};
...
Injector.prototype.withContextPath = function(contextPath, parentModule) {
  log.trace("withContextPath", contextPath);
  var clone = this.clone();
  clone.moduleFinder.setContextPath(contextPath);
  clone.moduleFinder.setParentModuleName(parentModule);
  return clone;
};

Injector.prototype.setContextPath = function(contextPath, parentModule) {
  this.moduleFinder.setContextPath(contextPath);
...
Injector.prototype.applyMap = function(bindTo, map) {
  log("applyMap, contextPath = %s", this.moduleFinder.contextPath);
  var self = this;
